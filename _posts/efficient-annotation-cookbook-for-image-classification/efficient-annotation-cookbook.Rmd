...
#Andrew: Any statistics on how much does ImageNet or MSCOCO cost? Maybe show in barplot.
...
However, they only perform experiments in the small-scale setting.
We ask ourselves: **How many number of annotations can we possibly reduce to annotate a large scale image classification dataset, such as ImageNet?**

### Testbed: ImageNet100 Sandbox
Evaluating and ablating multi-class label annotation efficiency at scale requires large datasets with diverse and relatively clean labels. We construct multiple subsets of the ImageNet dataset [@russakovsky2015imagenet] for our experiments. The following table shows the details of different subset in ImageNet100 Sandbox. 
...
Prior work [@hua2013collaborative; @long2015multi] simulate workers as confusion matrices. Class confusion was modeled with symmetric uniform noise, which can result in *over-optimistic* performance estimates. Human annotators exhibit asymmetric and structured confusion *i.e.*, classes get confused with each other differently. In Fig.\@ref(fig:structured-workers), we compares the number of annotations per image in simulation using uniform label noise vs. structured label noise that we crowdsource. We see large gaps between the two. This arises particularly when using learnt models in the loop, due to sensitivity to noisy labels coming from structured confusion in the workers. Therefore, we use simualted workers with structured noise in ImageNet100 Sandbox.
...
In the following, we show how each component affect the final efficiency:
...
```{r semi-supervised, fig.cap="Incorporating semi-supervised approach consistently increase the efficiency. Note that semi-supervised learning does not have siginificant boosting in Dog subset due ot the poor worker quality (43%). When eyeballing the subset, we also find ineligible label errors in the Dog subset.", echo=FALSE, layout="l-body-outset"}
knitr::include_graphics("assets/paper_figures/plot_semi.pdf")
```

### Self-Supervised Learning Advances Online-Labeling
With recent advances in self-supervised learning, it is feasible to learn strong image feature extractors that rival supervised learning, using pretext tasks without any labels. This allows learning in-domain feature extractors for annotation tasks, as opposed to using features pre-trained on ImageNet
...
```{r self-supervised, fig.cap="The improvements in self-suerpvised learning can be translated seamingly to online labeling. Note that no semi-supervised tricks are applied in this figure.", echo=FALSE, layout="l-body-outset"}
...
The validation set plays an important role of online-labeling. It is used to perform model selection and model calibration.
Prior work [@Branson_2017_CVPR] uses a modified cross-validation approach to generate model likelihoods. We find that this could underperform when the estimated labels are noisy, which pollutes validation set and makes calibration challenging. Instead, we propose to use the clean prototype images as the validation set. In our paper, we use 10 prototype images per class. 
We perform 3-fold cross validation in this experiment. 
When not using cross validation, we either randomly select a subset as validataion set or use the (clean) prototype images as validataion set. 
In Fig.\@ref(fig:clean-validation), we ablate the importance of having clean validation set and performing cross-validation in terms of accuracy and expected calibration error on the most challenging subset, the Dog subset.

```{r clean-validation, fig.cap="The validation set plays an important role of online-labeling. It is used to perform model selection and model calibration. We compare the importance of using clean examples as validation set. When not using clean validation set, the model tends to produce poorly calibrated probability (w/ calibration method applied [@guo2017calibration]), resulting in poor accuracy.", echo=FALSE, layout="l-body"}
...
To consider the worker identity, the task designer needs to query several gold standard questions from each worker.
...
```{r prior-change, fig.cap="For fine-grained dataset, it is usually worth using gold standard questions to get a better prior over worker skills. The number appended in the legend denote the prior strength.",  echo=FALSE, layout="l-body-outset"}
knitr::include_graphics("assets/paper_figures/plot_prior_change-10.pdf")
```

### Tradeoff between number of workers and time cost

One way to speed up the dataset annotation process is to hire more workers at the same time. However, under fixed number of total annotation, having more workers means having fewer observations for each workers, resulting in poor worker skill estimation.
We explore this tradeoff by manipulating the number of workers involved in Fig.\@ref(fig:number-workers). 
The gap is surprisingly high in fine-grained dataset (14% accuracy points difference in Dog subset), while there is nearly no tradeoff in coarse-grained dataset, such as Commodity subset.


```{r number-workers, fig.cap="Hiring more workers save the total time to annotate a dataset, while it sacrifices the accuracy of the dataset sometimes. For fine-grained dataset, the gap between using 10 workers and 1000 workers is around 14 accuracy points, while in coarse-grained dataset, there is nearly no tradeoff.",  echo=FALSE, layout="l-body-outset"}
...
```{r distraction-image, out.width="50%", fig.cap="We intentially add some irrelevance images from other classes to mimic the real-world cases. In our experiments, we find that even with 100% more images from irrelevant classes, we can still retain comparable efficiency.",  echo=FALSE, layout="l-body"}
...
2) How does one accelerate labeling beyond semantic classes, such as classifying viewing angle of a car?
...
knitr::include_graphics("assets/thumbnail-4.png")
```
